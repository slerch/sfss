...
uwind_fc <- matrix(NA, nrow = dim(uwind)[4], ncol = dim(uwind)[3])
vwind_fc <- matrix(NA, nrow = dim(uwind)[4], ncol = dim(uwind)[3])

library(akima) # for bilinear()
for(mm in 1:dim(uwind)[3]){
  for(ll in 1:dim(uwind)[4]){
    # bilinear interpolation, note exchange in 2nd dimension due to order of latitudes
    uwind_fc[ll,mm] <- bilinear(x = 8:9, y = 49:50, z = uwind[2:3,2:1,mm,ll], x0 = coordHD[1], y0 = coordHD[2])$z
    vwind_fc[ll,mm] <- bilinear(x = 8:9, y = 49:50, z = vwind[2:3,2:1,mm,ll], x0 = coordHD[1], y0 = coordHD[2])$z
  }
}
...
save(ensfc2015, dates2015, file = paste0(data_dir, "ensfc2015_interpolated.Rdata"))
...
save(ensfc2016, dates2016, file = paste0(data_dir, "ensfc2016_interpolated.Rdata"))
...
data_dir <- "/home/sebastian/Dropbox/ERC SummerSchool/software_tutorials/postproc_example/data/"
load(paste0(data_dir, "ensfc2015_interpolated.Rdata"))
load(paste0(data_dir, "ensfc2016_interpolated.Rdata"))

fcdates <- c(dates2015, dates2016)
ensfc <- rbind(ensfc2015, ensfc2016)

save(fcdates, ensfc, file = paste0(data_dir, "HDwind_ensfc_interpolated.Rdata"))
